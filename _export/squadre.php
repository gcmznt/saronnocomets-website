<?php
    ...
    foreach ($DataMining as $MininingRules) {
        ...
        while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
            ...
            $dataOutput[$CurrentFields['stagione']][$CurrentFields['squadra']]['giocatori'][] = $MininingRules['callback']($CurrentFields);
        }
    ...
    foreach($dataOutput AS $stagione => $squadre) {
        foreach($squadre AS $nome => $squadra) {
            $foto = '/_static/img/squadre/' . $nome . $stagione . '.jpg';
            if (is_file(__DIR__ . '/..' . $foto)) $dataOutput[$stagione][$nome]['foto'] = 'http://' . $_SERVER["HTTP_HOST"] . $foto;
            $foto = '/_static/img/squadre/' . $nome . $stagione . '_sm.jpg';
            if (is_file(__DIR__ . '/..' . $foto)) $dataOutput[$stagione][$nome]['foto_sm'] = 'http://' . $_SERVER["HTTP_HOST"] . $foto;
        }
    }
